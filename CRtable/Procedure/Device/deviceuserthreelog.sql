BEGIN
delete from device_user_threedays_nologin where datetime = adddate(date(sysdate()) ,-1);
INSERT INTO device_user_threedays_nologin(userid,username,logonname,schoolname,gradename,
...
AND UNIX_TIMESTAMP(a1.datetime)>=UNIX_TIMESTAMP(adddate(date(sysdate()) ,-3))
...
AND UNIX_TIMESTAMP(a2.datetime)>=UNIX_TIMESTAMP(adddate(date(sysdate()) ,-3))
