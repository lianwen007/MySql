...
st3.school_id,st0.book_id,st0.subject_id,st0.class_id,st3.class_name,
st0.student_id,st3.student_name,st0.integral,st0.qst_num,st0.qst_right,
st0.qst_right_rate,st0.is_weekly,
from_unixtime(unix_timestamp(),'yyyyMMdd') date_time
FROM(
-- onweek records
SELECT
sta1.book_id,sta1.subject_id,sta1.class_id,
...
FROM(
SELECT a1.student_id,a1.class_id,
...
CASE WHEN a2.book_type IN (1,3) THEN COUNT(a1.game_id)*10
    ...
    END AS qst_num,
CASE WHEN a2.book_type IN (1,3) THEN SUM(question_right)*100
...
GROUP BY a1.student_id,a1.class_id,a1.book_id,a2.subject_id,a2.book_type
) sta1
...
WHERE ta3.src_file_day = from_unixtime(unix_timestamp()-60*60*24*1,'yyyyMMdd')
) st3 
ON st0.student_id = st3.student_id limit 40;
