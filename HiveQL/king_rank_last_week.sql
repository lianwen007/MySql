CREATE TABLE rptdata.fact_king_rank_last_week(
  id string,
  school_id string,
  book_id string,
  subject_id string,
...
insert overwrite table rptdata.fact_king_rank_last_week partition(date_time)
...
FROM_UNIXTIME((unix_timestamp(substr(from_unixtime(unix_timestamp()),1,4),'yyyy')+
          ((weekofyear(from_unixtime(unix_timestamp(),'yyyy-MM-dd'))-1)*86400*7)),'yyyyMMdd') date_time
FROM(
...
WHERE a5.date_time = FROM_UNIXTIME((unix_timestamp(substr(from_unixtime(unix_timestamp()),1,4),'yyyy')+
          ((weekofyear(from_unixtime(unix_timestamp(),'yyyy-MM-dd'))-2)*86400*7)),'yyyyMMdd')
)stab2
