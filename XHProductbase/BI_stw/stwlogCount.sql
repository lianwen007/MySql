...
FROM
	(
		...
		WHERE
			a1.udatetime >= unix_timestamp(
				from_unixtime(
					unix_timestamp() - 60 * 60 * 24 * 0,
					'yyyy-MM-dd'
				),
				'yyyy-MM-dd'
			)
...
) sta2 ON sta2.userid = sta1.userid
AND sta2.bookid = sta1.bookid
...
LEFT JOIN (
	SELECT DISTINCT
		...
		a4.credit
	FROM
		xh_basecount.product_stw_base a4
	JOIN (
		SELECT
			userid,
			max(updatetime) AS maxtime
		FROM
			xh_basecount.product_stw_base
		WHERE
			udatetime >= unix_timestamp(
				from_unixtime(
					unix_timestamp() - 60 * 60 * 24 * 0,
					'yyyy-MM-dd'
				),
		...
		GROUP BY
			userid
	) a5 ON a4.userid = a5.userid
	AND a4.updatetime = a5.maxtime
	WHERE
		a4.udatetime >= unix_timestamp(
			from_unixtime(
				unix_timestamp() - 60 * 60 * 24 * 0,
				'yyyy-MM-dd'
			),
...
LEFT JOIN (
	SELECT DISTINCT
		...
		a6.newintegral
	FROM
		xh_basecount.product_stw_base a6
	JOIN (
		SELECT
			userid,
			bookid,
		...
		GROUP BY
			userid,
			bookid
	) a7 ON a6.userid = a7.userid
	AND a6.bookid = a7.bookid
	AND a6.updatetime = a7.maxtime
	WHERE
		a6.udatetime >= unix_timestamp(
			from_unixtime(
				unix_timestamp() - 60 * 60 * 24 * 0,
				'yyyy-MM-dd'
			),
...
) sta6 ON sta1.userid = sta6.userid
AND sta1.bookid = sta6.bookid;
